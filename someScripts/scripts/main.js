...
var timerID = -1;
var pVersion = "5.1.0.12";     //plugins version
var macVersion = "5.2.0.1";
...
var playerVideo;
var playerAudio;
var bFirstLogin = true; // 切换分辨率标志，切换过标为true,H5每切换一次分辨率需要重新打开视频一次
var H5Live = 1; // 0: flash, 1: h5
var H5Live_Edge = 0; //Edge 浏览器 0:flash, 1:h5
var H5Live_Safari = 1; //Safari 浏览器 0:npapi, 1:h5
var inLivePage = false;
...
function isLoginByMainStream() {
	return gVar.nStreamType == 0;
}
...
function mStarDevice() {
	return gVar_first.model >= 7001 && gVar_first.model < 8000;
}
/*
...
function CgiCallNoMask(CallBack, Paop, cmd, xml, timeout, usertype) {
	debug("CgiCallNoMask", "CgiCallNoMask cmd = " + cmd);
    if (typeof timeout == 'undefined') timeout = 5000;
    gVar.errTitle = Paop;
    if (!jQuery.isFunction(CallBack)) {
        CallBack = function () {
...
function RfParamCallNoShadow(CallBack, Paop, cmd, xml, timeout, usertype) {
	...
	debug("RfParamCallNoShadow", "no shadow cmd = " + cmd);
...
isEncy[0] = true;
isOpenV[0] = true; // Device 0 open status is true
	...
	function SPcontrol(e, p, tag, cmd){	//viedo all open OR all close
		...
		if(e.type == "mousedown"){
			...
			if($p.attr("class") == "liveBtnBt1" || $p.attr("class") == "liveBtnBt2"){
				try{
					debug("SPcontrol", "IFs=" + IFs + " gVar.ip=" + gVar.ip + " cmd=" + cmd);
					gDvr.VideoPlay(IFs, gVar.ip, cmd);
				}catch(e){}
				if (cmd==3) {
...
//				        try{gDvr.TalkCMD(IFs, 1);}catch(e){}
                    ...
                    if(IsFirefox() || IsChromeSupportNacl() || IsSarfariSupportH5()){
                    	if(H5Live){	
                        	flv_destroy();
                    	}
                    }
	...
	$(function () {
	    $("#LiveMenu").click(function () {	//live
			...
			if (mStarDevice() && isLoginByMainStream()) {
				if (!$.browser.msie && browserVer.browsers != "IE11") {
					debug("LiveMenu.Click", "It is not ie, In live page, will call getMainStreamFormat");
					RfParamCall(videoMask_call_1, "", "getMainStreamFormat");
				} else {
					gVar.ChangPage(1);
				}
			} else {
        ...
        try {
            ...
            if (needServiceUpdate) {
	           ...
	           // obj.innerHTML = '<a id="down" href="IPCWebComponents.exe">' + lg.get("IDS_PLUGIN_UPDATE_TIP") + '</a>';
        		...
        		obj = document.getElementById("downPlugins");
        		obj.innerHTML = '<a id="down" href="IPCWebComponents.exe">' + lg.get("IDS_PLUGIN_UPDATE_TIP") + '</a>';
			}
...
function LoadLeftPage(func){
	...
	$.get("html/left.html?"+gVar.nDate, function(data){
		...
		lan("left");
		RegLeftEvent();
	    if (IsChromeSupportNacl() || IsFirefox() || IsEdge() || IsSarfariSupportH5()) {
	        $("#PlayBackMenu").remove();
	        $("#bs_multi").remove();
	        $("#multiLive").remove();
	        // $("#sys_dr").remove();
...
function LoadLivePage() {
    ...
    LoadLeftPage(function () {
		...
		if (!mStarDevice() || $.browser.msie || browserVer.browsers == "IE11" || !isLoginByMainStream()) {
			if (gVar.nOpenPreView) {
				debug("LoadLeftPage", "It is not m device, or not ie, click liveBtnBt1(open?) button");
				$(".liveBtnBt1").mousedown();
				//gDvr.OpenAudio(IFs);
				gVar.bliveOpen = true;
			} else {
				debug("LoadLeftPage", "It is not m device, or not ie, click liveBtnBt2(close?) button");
		...
		if (mStarDevice() && isLoginByMainStream()) {
			debug("LoadLeftPage", "it is m device, send getMainStreamFormat");
			RfParamCall(videoMask_call, "", "getMainStreamFormat");
		}
		...
		if (browserVer.browsers == "Internet Explorer" || browserVer.browsers == "IE11" || (browserVer.browsers == "safari" && !H5Live_Safari)) {
	        for (var i = 1; i < 9; i++) {
...
function reopenVideo () {
	debug("reopenVideo", "reopenVideo");
	$(".liveBtnBt1").mousedown();
	gVar.bliveOpen = true;
	cutdownalarmrecord[0] == true;			
}

function videoMask_call_1 (xml) {
	debug("videoMask_call_1", "streamType = " + $(xml).find("result").text());
	if ($(xml).find("result").text() != "" && 
		$(xml).find("streamType").text() != "" && 
		$(xml).find("streamType").text()*1 == 1) {
		
		cleanVideo();
		gVar.ChangPage(1, false);
	}
	else {
		gVar.ChangPage(1, true);
	}
	
	MasklayerHide();		
}

function videoMask_call (xml) {
	debug("videoMask_call", "videoMask_call");
	
	if (!$.browser.msie && browserVer.browsers != "IE11") {
		debug("videoMask_call not ie, streamType = " + $(xml).find("result").text());
		if ($(xml).find("result").text() != "" && 
			$(xml).find("streamType").text() != "" && 
			$(xml).find("streamType").text()*1 == 1) {
			...
			// When compressor is h264+,  playVideo button don't click,
			// isOpenV[0] is false, when change page and back to live page,
			// videoPlay interface can't be called, so we set isOpenV[0] manually
			isOpenV[0] = true;
		} else {
			if (gVar.nOpenPreView) {
				debug("videoMask_call", "gVar.nOpenPreView " + gVar.nOpenPreView);
				$(".liveBtnBt1").mousedown();
				//gDvr.OpenAudio(IFs);
				gVar.bliveOpen = true;
			} else {
				debug("videoMask_call", "gVar.nOpenPreView 1 " + gVar.nOpenPreView);
				$(".liveBtnBt2").mousedown();
				gVar.bliveOpen = false;
				cutdownalarmrecord[chid] == false;
			}	
...
function SelectChannel(ch, chQx, chDev, ip, fromEventId) {
	debug("SelectChannel", "fromEventId=" + fromEventId);
    ...
    if (typeof chDev != 'undefined') {
		...
		debug("SelectChannel", "check r2plus and call naa, frome id =" + fromEventId);
		if (isR2Plus()) 
			$("#guard_6").css("display", "none");
	
		if(gVar.sPage == 1/* && fromEventId == 100*/){
			setTimeout(function (){
				debug("SelectChannel", "send getNetworkAutoAdaptability");
				CgiCallNoMask(getNAACall,"", "getNetworkAutoAdaptability");
			},300);
		}
		debug("SelectChannel", "check r2plus and call naa complete");
    }
...
function PluginCallBack() {
    msgID = arguments[0];
    debug("PluginCallBack", "msgID : "+msgID);
	switch(msgID){
	    case 100:
            {
	            ...
	            if (!gVar.bLogin) {
	                ...
	                if (arguments[69]) {
						...
						debug("case 100", "will call SelectChannel");
	                    SelectChannel(gVar.selCh, qx, gVar.selChDev, gVar.ip, 100);
	                    /*if (qx < 100) {         //H264
	                        RfQx((Qqx > qx ? qx : Qqx));
	                   }else {          //MJ
	                        RfQx(qx);
...
//        isOpenAlarmR[arguments[1]*1]=0;
    ...
    case 500:
    {
        ...
        SelectChannel(gVar.selCh, gVar.selChQx, gVar.selChDev,gVar.ip, 500);
...
case 507: {
	if (!isLoginByMainStream())
		break;
	
	var index = arguments[1];
	
	debug("pluginCallback", "case 507 index = " + index);
	
	if (isOpenT[index]) gDvr.TalkCMD(index, 1);
	if (isOpenA[index]) gDvr.CloseAudio(index);
	if (isOpenV[index]) gDvr.VideoPlay(index, "", 4);
	
	cutdownalarmrecord[index] = false;
				
	//$(".liveBtnBt2").mousedown();
	setTimeout(function () {
		if (inLivePage) {
			debug("PluginCallback", "open=" + isOpenV[index] + " index=" + index + " ip=" + Struct.devInfo[index].ip);
			if (isOpenV[index]) 
				gDvr.VideoPlay(index, Struct.devInfo[index].ip, 3);
		}
...
function safariAutoplaySettingCheck (media) {
	var promise = media.play();
	if (promise !== undefined) {
		promise.then(function (value) {
			console.log("promise, safari allow autoplay");
		});
		
		promise.catch(function (value) {
			console.log("promise, safari deny autoplay e=" + value);
		});
	}
}

//
// Sarfari on macOS, media element often pause, maybe it has no buffer to play, but it can't 
// play after new media data arrived, so a timer check video and audio to make them to play
//
function Flv_load_video () {
	//console.log("-------------------Flv_load_video--------------------");
    ...
    var playervideoinit =setInterval(function() {
            if (playerVideo == null) {
		playerVideo = flvjs.createPlayer(mediaDataSource, {
		    ...
		    stashInitialSize: 128
                   ...
                   clearInterval(playervideoinit);
                   playvideoNextProcess();
            }
...
function playvideoNextProcess(){
        var element = document.getElementsByName('videoElement')[0];
        playerVideo.attachMediaElement(element);
		
		// Call setTimeout for that make safari to buffer media in 2s, 
		// setInerval is for keeping player to play. 
		if (IsSarfariSupportH5()) {
			element.oncanplay = function() {
				//console.log("video can play");
				safariAutoplaySettingCheck(playerVideo);
					...
					videoTimer = setInterval(function() {
						if (element.pause) {
							...
							for (var i = 0; i < buffered.length; i++) {
								...
								//console.log("video paused, start=" + start + " end=" + end + " currenttime=" + element.currentTime);
								this.playerVideo.play();
							}
							
							if (end - start > 2)
								this.playerVideo._internalSeek(buffered.end(0) - 1);
						}
...
function Flv_load_audio () {
    ...
    xhr.onload = function (e) {
        ...
        var playeraudioinit =setInterval(function() {
            if (playerAudio == null) {
		playerAudio = flvjs.createPlayer(mediaDataSource, {
		    ...
		    stashInitialSize: 128
                 ...
                 clearInterval(playeraudioinit);
                 playaudioNextprocess();

             }
         },100);
}

function playaudioNextprocess()	{
		...
		if (IsSarfariSupportH5()) {
			element.oncanplay = function() {
				...
				safariAutoplaySettingCheck(playerAudio);
					...
					audioTimer = setInterval(function() {
						if (element.pause) {
							...
							for (var i = 0; i < buffered.length; i++) {
								var start = buffered.start(i);
								var end = buffered.end(i);
								//console.log("audio paused, start=" + start + " end=" + end + " currenttime=" + element.currentTime);
								this.playerAudio.play();
							}
							
							if (end - start > 5)
								this.playerAudio._internalSeek(buffered.end(0) - 1.5);
						}
...
function flv_destroy() {
	...
	try {
		...
		//console.log("-------------flv_destory complete--------------------");
	} catch (e) {
